...
- [x] 4.6 `android-release` job 上傳成功後以 `stefanzweifel/git-auto-commit-action@v7` 將 bumped pubspec commit 回 `main`，commit message 使用正體中文 Conventional Commits：`chore(mobile): bump build number 至 +<N+1>`，body 列點並含 `[skip ci]`；實作 Decision: versionCode 以 pubspec `+buildNumber` 為唯一來源，CD 遞增後 commit 回 main 並加 `[skip ci]`
