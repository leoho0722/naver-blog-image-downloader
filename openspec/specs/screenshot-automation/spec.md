...
定義 mobile app 的 screenshot 自動化框架：包含 debug-only screenshot mode、單一來源的 scenario manifest，以及跨語系 / 主題的 Maestro 批次截圖矩陣。此 capability 讓商店素材可以跨語系 / 主題穩定產出，並讓 UI 變更能以實機拍攝的方式回歸驗證，避免每次人工切換畫面狀態。
