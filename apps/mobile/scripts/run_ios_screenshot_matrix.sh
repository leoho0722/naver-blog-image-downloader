#!/bin/bash
# 直接透過 xcrun simctl 對 iOS 模擬器逐一啟動 screenshot mode 並擷取畫面。
#
# 需求：
#   - 指定 iOS 模擬器已 boot
...
echo "模擬器：$SIM_ID" | tee -a "$LOG_FILE"
echo "Bundle：$BUNDLE_ID" | tee -a "$LOG_FILE"
echo "輸出目錄：$OUT_DIR" | tee -a "$LOG_FILE"

# 先確認模擬器處於 Booted 狀態，否則直接終止
...
# 從 JSON 讀取每個 scenario 的 id 與 iOS 等待秒數，以 "id:wait" 單筆字串儲存
SCENARIO_ENTRIES=()
...
for locale in "${LOCALES_FILTERED[@]}"; do
  for theme in "${THEMES_FILTERED[@]}"; do
    ...
    for entry in "${SCENARIO_ENTRIES_FILTERED[@]}"; do
      ...
      # 先確保 app 停止再啟動，避免前一次狀態殘留
      xcrun simctl terminate "$SIM_ID" "$BUNDLE_ID" >/dev/null 2>&1 || true

      LAUNCH_OUT=$(xcrun simctl launch "$SIM_ID" "$BUNDLE_ID" \
        -screenshotMode true \
        -scenario "$scenario" \
        -locale "$locale" \
        -theme "$theme" 2>&1)
      ...
      if xcrun simctl io "$SIM_ID" screenshot "$OUT_PATH" >/dev/null 2>&1; then
        if [ -s "$OUT_PATH" ]; then
          SUCCESS=$((SUCCESS+1))
